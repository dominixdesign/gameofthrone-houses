      ...
      <div class="overflow-x-auto mt-6">
        <table class="table-auto border-collapse w-full">
          ...
          <tbody class="text-sm font-normal text-gray-700">
            <tr v-for="house in apidata" :key="house.name" class="hover:bg-gray-100 border-b border-gray-200 py-10">
              <td class="px-4 py-4">
                {{ house.name }}
              </td>
              <td class="px-4 py-4">
...
export default {
  async asyncData ({ $axios }) {
    const apidata = await $axios.$get('https://www.anapioficeandfire.com/api/houses')
    return { apidata }
  }
}
</script>
